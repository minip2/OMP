...
%>
	<tr>
		...
		<%if(request.getParameter("productId").equals(productId)) {%>
		<td><%switch(request.getParameter("categoryVal")) { 
	case "1000" : %>
<select name = 'categoryVal'>
	 		<option>상품분류</option>
	...
	default :%>
		...
		<td><%=productId%></td>
		<td><input type = "txet" name = 'productName' value="<%= productName%>" size="8"></td>
		<td><input type = "txet" name='quantity' value="<%= quantity%>"size="8"></td>
		<td><input type = "txet" name = 'price' value="<%= price%>"size="8"></td>
